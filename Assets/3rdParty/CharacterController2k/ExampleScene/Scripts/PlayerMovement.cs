...
#pragma warning restore CS0109 // member does not hide accessible member
    ...
    bool EventUnderWater()
    {
        ...
        if (inWater) // in water and valid water collider?
        {
            ...
            return !Utils.RaycastWithout(origin, Vector3.down, out RaycastHit hit, distance, gameObject,
                canStandInWaterCheckLayers);
        }
    ...
    MoveState UpdateWALKINGandRUNNING(Vector2 inputDir, Vector3 desiredDir)
    {
        ...
        horizontalSpeed = AccelerateSpeed(inputDir, horizontalSpeed, speed,
            inputDir != Vector2.zero ? walkAcceleration : walkDeceleration);
        moveDir.x = desiredDir.x * horizontalSpeed;
        moveDir.y = ApplyGravity(moveDir.y);
        moveDir.z = desiredDir.z * horizontalSpeed;
    ...
    MoveState UpdateCROUCHING(Vector2 inputDir, Vector3 desiredDir)
    {
        ...
        // move with acceleration (feels better)
        horizontalSpeed = AccelerateSpeed(inputDir, horizontalSpeed, crouchSpeed,
            inputDir != Vector2.zero ? crouchAcceleration : crouchDeceleration);
        moveDir.x = desiredDir.x * horizontalSpeed;
        moveDir.y = ApplyGravity(moveDir.y);
        moveDir.z = desiredDir.z * horizontalSpeed;
    ...
    MoveState UpdateCRAWLING(Vector2 inputDir, Vector3 desiredDir)
    {
        ...
        // move with acceleration (feels better)
        horizontalSpeed = AccelerateSpeed(inputDir, horizontalSpeed, crawlSpeed,
            inputDir != Vector2.zero ? crawlAcceleration : crawlDeceleration);
        moveDir.x = desiredDir.x * horizontalSpeed;
        moveDir.y = ApplyGravity(moveDir.y);
        moveDir.z = desiredDir.z * horizontalSpeed;
    ...
    MoveState UpdateAIRBORNE(Vector2 inputDir, Vector3 desiredDir)
    {
        ...
        // move with acceleration (feels better)
        horizontalSpeed = AccelerateSpeed(inputDir, horizontalSpeed, speed,
            inputDir != Vector2.zero ? airborneAcceleration : airborneDeceleration);
        moveDir.x = desiredDir.x * horizontalSpeed;
        moveDir.y = ApplyGravity(moveDir.y);
        moveDir.z = desiredDir.z * horizontalSpeed;
    ...
    MoveState UpdateSWIMMING(Vector2 inputDir, Vector3 desiredDir)
    {
        ...
        // move with acceleration (feels better)
        horizontalSpeed = AccelerateSpeed(inputDir, horizontalSpeed, swimSpeed,
            inputDir != Vector2.zero ? swimAcceleration : swimDeceleration);
        moveDir.x = desiredDir.x * horizontalSpeed;
        moveDir.z = desiredDir.z * horizontalSpeed;
    ...
    void FixedUpdate()
    {
        ...
        if (animator != null)
        {
            float runCycle = Mathf.Repeat(animator.GetCurrentAnimatorStateInfo(0).normalizedTime + runCycleLegOffset,
                1);
